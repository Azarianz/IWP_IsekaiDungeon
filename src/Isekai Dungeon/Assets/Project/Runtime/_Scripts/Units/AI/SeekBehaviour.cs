...
public class SeekBehaviour : SteeringBehaviour
{
    ...
    public override (float[] danger, float[] interest) GetSteering(float[] danger, float[] interest, AIData aiData)
    {
        ...
        //Cache the last position only if we still see the target (if the targets collection is not empy)
        ...
        Vector3 directionToTarget = (targetPositionCached - (Vector3)transform.position);

        for(int i =0; i < interest.Length; i++)
        {
            float result = Vector3.Dot(directionToTarget.normalized, Directions.eightDirections[i]);

            //accept only directions at the less than 90 degress o the target direction
            if(result > 0)
            {
